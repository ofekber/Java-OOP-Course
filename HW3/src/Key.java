...
public class Key extends Item {
    ...
    public void useItem(Player player) {
        if (isItemNear(this, player) && !player.getCurrentRoom().isKeyUsed()) {
            System.out.println(player.getNameOfPlayer() + " used " + itemEquals(this, player).getNameOfItem() + " in " + player.getCurrentRoom().getNameOfRoom() + ".");
            player.getCurrentRoom().setKeyUsed(true);
            player.getCurrentRoom().setStatusOfPuzzle(false);
            player.getCurrentRoom().setPlaceForKey(this);
            transferKey(player);
        } else if (isItemNear(this, player) && player.getCurrentRoom().isKeyUsed()) {
            System.out.println(player.getCurrentRoom().getNameOfRoom() + " was already unlocked.");
