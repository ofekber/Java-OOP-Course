...
public class GameManager {
    ...
    public void removeRoom(Room room) {
        if (!isLegal(room)) {
        ...
        } else if (isLegal(room) && player != null && this.player.getCurrentRoom() == room) {
            System.out.println(room.getNameOfRoom() + " could not be removed.");
        } else {
            ...
            for (int i = 0; i < MAX_ROOMS; i++) {
                if (this.rooms[i] == room) {
                    this.rooms[i] = null;
                    System.out.println(room.getNameOfRoom() + " was removed from the game.");
                    break;
                }
            }
        }
    ...
    public void addItem(Room room, Item item) {
        ...
        for (int i = 0; i < MAX_ROOMS; i++) {
            if (this.rooms[i] == room && rooms[i].getNumberOfItems() < 2) {
