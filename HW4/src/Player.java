...
public class Player implements Cloneable {
    ...
    private Bag bag;
    private QuartNode<Room> currentRoom;
    ...
    public int indexOfItemInRoom(Item item) {
        if (this.currentRoom.getValue().getItems()[0] != null && this.currentRoom.getValue().getItems()[0].equals(item)) {
            return 0;
        } else if (this.currentRoom.getValue().getItems()[1] != null && this.currentRoom.getValue().getItems()[1].equals(item)) {
            return 1;
        }
        return -1;
    }
    ...
    public void pickUpItem(Item item) {
        ...
        boolean itemIsinRoom = ((this.currentRoom.getValue().getItems()[0] != null && this.currentRoom.getValue().getItems()[0].equals(item)) || (this.currentRoom.getValue().getItems()[1] != null && this.currentRoom.getValue().getItems()[1].equals(item)));
                ...
                for (int i = 0; i < this.getBag().inventory.length; i++) {
                    if (this.getBag().inventory[i] == null) {
                        if (itemIsinRoom) {
                            ...
                            System.out.println(this.nameOfPlayer + " picked up " + item.getNameOfItem() + " from " + this.currentRoom.getValue().getNameOfRoom() + ".");
                            this.currentRoom.getValue().setItem(null, indexOfItemInRoom(item));
                            break;
                        } else {
                            System.out.println(item.getNameOfItem() + " is not in " + this.currentRoom.getValue().getNameOfRoom() + ".");
                            break;
                        }
                    }
    ...
    public void dropItem(Item item) {
        ...
        for (int i = 0; i < bag.inventory.length; i++) {
            if (bag.inventory[i] != null && bag.inventory[i].equals(item)) {
                itemInBag = true;
                if (this.getCurrentRoom().getValue().getItems()[0] == null) {
                    roomIsFull = false;
                    getCurrentRoom().getValue().setItem(item, 0);
                    System.out.println(this.getNameOfPlayer() + " dropped " + bag.inventory[i].getNameOfItem() + " in " + this.currentRoom.getValue().getNameOfRoom() + ".");
                    bag.inventory[i] = null;
                    break;
                } else if (this.getCurrentRoom().getValue().getItems()[1] == null) {
                    roomIsFull = false;
                    getCurrentRoom().getValue().setItem(item, 1);
                    System.out.println(this.getNameOfPlayer() + " dropped " + bag.inventory[i].getNameOfItem() + " in " + this.currentRoom.getValue().getNameOfRoom() + ".");
                    bag.inventory[i] = null;
                    break;
                }
        ...
        if (!itemInBag) {
            ...
            System.out.println(this.currentRoom.getValue().getNameOfRoom() + " is full.");
        }
    }
    ...
    public void disassembleItem(Item item) {
        ...
        if (this.currentRoom.getValue().getItems()[0] != null && this.currentRoom.getValue().getItems()[0].equals(item)) {
            System.out.println(this.nameOfPlayer + " disassembled " + this.currentRoom.getValue().getItems()[0].getNameOfItem() + ".");
            this.currentRoom.getValue().setItem(null, 0);
            wasDissasembled = true;
        } else if (this.currentRoom.getValue().getItems()[1] != null && this.currentRoom.getValue().getItems()[1].equals(item)) {
            System.out.println(this.nameOfPlayer + " disassembled " + this.currentRoom.getValue().getItems()[1].getNameOfItem() + ".");
            this.currentRoom.getValue().setItem(null, 1);
            wasDissasembled = true;
     ...
     */
    public QuartNode<Room> getCurrentRoom() {
        return currentRoom;
    }
     ...
     */
    public void setCurrentRoom(QuartNode<Room> currentRoom) {
    ...
    public void solvePuzzle() {
        if (!this.currentRoom.getValue().isStatusOfPuzzle()) {
            System.out.println("There is no active puzzle in " + this.currentRoom.getValue().getNameOfRoom() + ".");
        } else {
            System.out.println(this.nameOfPlayer + " is solving the puzzle in " + this.currentRoom.getValue().getNameOfRoom() + ".");
            this.getCurrentRoom().getValue().setStatusOfPuzzle(false);
        }
    }
     ...
     *
     * @return A new Player object that is an exact copy of this Player,
     *         including deep copies of its associated Bag and current Room.
     *
     */
    @Override
    public Player clone() {
        try {
            ...
            copy.nameOfPlayer = this.nameOfPlayer;
            copy.bag = this.bag.clone();
            if (this.currentRoom == null) {
                copy.currentRoom = null;
            } else {
                copy.currentRoom = this.currentRoom.clone();
            }
            return copy;
