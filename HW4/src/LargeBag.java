...
public class LargeBag extends Bag implements Cloneable {
    ...
    public void useItem(Player player) {
        ...
        if (!isItemNear(this, player)) {
            ...
            if (isOldLargeBag) {
                ...
                player.setBag(this);
                player.getCurrentRoom().getValue().setItem(tmp, player.indexOfItemInRoom(this));
     ...
     *
     * @return A new LargeBag object that is an exact copy of this LargeBag,
     *         including a deep copy of the items contained within it.
     *
     * @see Item#clone()
     */
    @Override
    public LargeBag clone() {
        LargeBag copy = (LargeBag) super.clone();
        copy.nameOfItem = this.nameOfItem;
        copy.inventory = new Item[copy.getSizeOfBag()];

        for (int i = 0; i < sizeOfBag; i++) {
            if (this.inventory[i] != null) {
                copy.inventory[i] = this.inventory[i].clone();
            }
        }
        return copy;
    }
