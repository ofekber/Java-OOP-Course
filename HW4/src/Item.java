...
public abstract class Item implements Cloneable {
    ...
    public boolean isItemInRoom(Item item, Player player) {
        ...
        if (player.getCurrentRoom().getValue().getItems()[0] != null && player.getCurrentRoom().getValue().getItems()[0].equals(item) || player.getCurrentRoom().getValue().getItems()[1] != null && player.getCurrentRoom().getValue().getItems()[1].equals(item)) {
            itemInRoom = true;
        }
        return itemInRoom;
    }
    ...
    protected Item itemEquals(Item item, Player player) {
        ...
        if (player.getCurrentRoom().getValue().getItems()[0] != null && player.getCurrentRoom().getValue().getItems()[0].equals(item)) {
            return player.getCurrentRoom().getValue().getItems()[0];
        }
        if (player.getCurrentRoom().getValue().getItems()[1] != null && player.getCurrentRoom().getValue().getItems()[1].equals(item)) {
            return player.getCurrentRoom().getValue().getItems()[1];
        }
        return item;
    }
    ...
    /**
     * Creates and returns a shallow copy of this Item object.
     * This method overrides the clone method of the Object class.
     *
     * @return A new Item object that is an exact copy of this Item,
     *         with the same nameOfItem property.
     *         Note that this method performs a shallow copy,
     *         meaning that any objects referenced by this item
     *         are not cloned; their references are copied.
     */
    @Override
